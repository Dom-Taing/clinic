...
export default function CheckIn({ clinicLocation }: CheckInProps) {
  ...
  const [dutyCheckInTime, setDutyCheckInTime] = useState("");
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        // Check if the user has already checked in duty today
        ...
        if (dutyData && dutyData.length > 0) {
          const lastCheckIn = dutyData[0].time;
          const lastDutyDate = new Date(lastCheckIn).toLocaleTimeString(
            "en-US",
            {
              day: "2-digit",
              month: "2-digit",
          ...
          setError(`You have already sign in for duty today ${lastDutyDate}`);
          setLoading(false);
          return;
        }
        ...
        const summaryMessage = `Doctor: ${
          currentUser.name_kh
        }\nDuty Checked in at: ${convertToDefault(currentDateTime)}`;
        ...
        setDutyCheckInTime(currentDateTime); // Set the check-in time
      } catch (error) {
        setError((prev) =>
          prev ? prev : "Unable to retrieve location or complete duty check-in."
        );
      } finally {
        setLoading(false);
      }
    ...
    <div>
      ...
      {error ? (
        ...
        <p>
          ...
          {new Date(dutyCheckInTime).toLocaleTimeString("en-US", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
            hour: "2-digit",
